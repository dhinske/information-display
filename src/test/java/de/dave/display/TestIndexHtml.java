...
public class TestIndexHtml {
	...
	public void testAddRows() {
		...
		Assert.assertEquals(1, indexHtml.getRow("row3").childNode(1).childNodeSize());
		indexHtml.addRow("row3", "Key2", "<font color=green>active</font>");
		Assert.assertEquals(2, indexHtml.getRow("row3").childNode(1).childNodeSize());
	}

	@Test
	public void testCreateElementsForValue() {
		Assert.assertEquals(null, indexHtml.createElementsForValue("value"));
		Assert.assertNotEquals(null, indexHtml.createElementsForValue("<font color=green>active</font>"));
		Assert.assertEquals(null, indexHtml.createElementsForValue("<fontsds<"));
		Assert.assertEquals(null, indexHtml.createElementsForValue("<<>"));
	}
}
